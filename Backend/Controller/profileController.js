...
function doSearchUserProfile(req,resp){
    profileModal.findOne({email:req.query.email})
    .then((result)=>{
...
function doUpdateProfile(req,resp)
{
    ...
    profileModal.updateOne({email:req.body.email},{$set:{name:req.body.name,address:req.body.address,mobile:req.body.mobile,village:req.body.village}}).then((result)=>{
